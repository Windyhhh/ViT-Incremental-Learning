...
class Config:
    ...
    EPOCHS_PER_TASK = 50  # 增加训练轮数 (从30→50,针对原型对齐瓶颈)
    ...
    # 多原型策略 (增强)
    NUM_PROTOTYPES_PER_CLASS = 5  # 增加原型数量 (从3→5)
    ...
    def refine_prototypes_in_merged_space(self, native_model, merged_model,
        ...
        for class_id in class_ids:
            if native_features_dict[class_id] and merged_features_dict[class_id]:
                ...
                # 迭代精炼
                for iteration in range(Config.PROTOTYPE_REFINEMENT_ITERATIONS):
                    # 在merged空间重新计算原型
...
def train_task(model, task_data, task_id, device, old_model=None, old_prototypes=None):
    ...
    if Config.LR_SCHEDULER == "cosine":
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=Config.EPOCHS_PER_TASK, eta_min=Config.LR * 0.01
        )
    else:
        scheduler = None
    ...
    for epoch in range(Config.EPOCHS_PER_TASK):
        ...
        pbar = tqdm(task_data['train_loader'],
                   desc=f"  Epoch {epoch+1}/{Config.EPOCHS_PER_TASK}")
        ...
        for images, labels in pbar:
            ...
            if old_model is not None and Config.ENABLE_FEATURE_DISTILLATION:
                ...
                loss = loss + Config.FEATURE_DISTILL_LAMBDA * distill_loss
